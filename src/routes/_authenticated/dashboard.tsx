...
function Dashboard() {
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        description="Welcome back to Continuum OS. Portfolio overview active."
        actions={
          <Button
            size="sm"
                ...
                <tbody>
                  {stats.invoices.map((inv: any) => (
                    <tr key={inv.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.03]">
                      <td className="px-6 py-3 font-mono text-xs text-[#D4AF37]">{inv.invoice_number}</td>
                      <td className="px-6 py-3 text-white/80">{inv.companies?.legal_name ?? "—"}</td>
                      <td className="px-6 py-3 text-white/40">{inv.due_date ?? "—"}</td>
                      <td className="px-6 py-3 text-right font-semibold text-white">{currency(inv.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {PIPELINE_ORDER.map(({ key, label }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-xs font-bold ${dim ? "text-white/30" : "text-[#D4AF37]"}`}>{count}</span>
                  </div>
                );
              })}
            </div>
              ...
              <ul className="divide-y divide-white/5">
                {stats.loans.map((l: any) => (
                  <li key={l.id} className="px-6 py-3 text-sm flex items-center justify-between">
                    <div>
                      <div className="font-medium text-white">{l.companies?.legal_name ?? l.lender}</div>
                      <div className="text-xs text-white/40">{l.lender} · {l.next_due_date}</div>
                    </div>
                    <div className="font-semibold text-[#D4AF37]">{currency(l.monthly_payment)}</div>
                  </li>
                ))}
              </ul>
            ) : (
...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-[#0d0d0d] rounded-2xl border border-white/5 shadow-xl overflow-hidden ${className}`}>
      {children}
    </div>
  );
}
